...
function validateSourceFields(
  ...
  if (type === "json-api") {
    ...
    if (
      !isRecord(mapping) ||
      !isNonEmptyString(mapping.title) ||
      !isNonEmptyString(mapping.url)
    ) {
      issues.push({
        campo: `${path}.mapping`,
  ...
  if (
    type === "repo-releases" &&
    (!Array.isArray(raw.repos) || raw.repos.length === 0)
  ) {
    issues.push({
      campo: `${path}.repos`,
...
function validateCaps(raw: unknown, issues: ValidationIssue[]): void {
  ...
  if (typeof raw.maxItems !== "number" || raw.maxItems <= 0) {
    issues.push({
      campo: "caps.maxItems",
      motivo:
        "declara el tope global de elementos enviados al modelo, un número mayor que cero",
    });
  }
  if (
    ...
    issues.push({
      ...
      motivo:
        "declara el tope por fuente en porcentaje del total, entre 0 (exclusivo) y 100",
    });
  }
}
